...
extern "C" cupdlp_int cuda_alloc_MVbuffer(
  ...
  CHECK_CUSPARSE(cusparseSpMV_bufferSize(handle, CUSPARSE_OPERATION_TRANSPOSE,
                                         &alpha, cuda_csc, vecY, &beta, vecATy,
                                         CudaComputeType, alg, &ATyBufferSize))
...
extern "C" cupdlp_int cuda_csr_Ax(cusparseHandle_t handle,
  ...
  CHECK_CUSPARSE(
      cusparseSpMV(handle, op, alpha, cuda_csr, vecX, beta, vecAx,
                   // CudaComputeType, CUSPARSE_SPMV_ALG_DEFAULT, dBuffer))
                   CudaComputeType, CUSPARSE_SPMV_CSR_ALG2, dBuffer))
...
extern "C" cupdlp_int cuda_csc_ATy(cusparseHandle_t handle,
  ...
  CHECK_CUSPARSE(
      cusparseSpMV(handle, op, alpha, cuda_csc, vecY, beta, vecATy,
                   // CudaComputeType, CUSPARSE_SPMV_ALG_DEFAULT, dBuffer))
                   CudaComputeType, CUSPARSE_SPMV_CSR_ALG2, dBuffer))
